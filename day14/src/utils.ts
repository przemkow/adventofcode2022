...
function fillRow(arr: boolean[][], start: [number, number], end: [number, number]) {
...
export async function getInput() {
  const map: boolean[][] = Array.from({length: 600}, () => {
    return Array.from({length: 600}, () => false)
  })
  ...
  rawInput.split(linebreak).forEach((row) => {
    const directionPairs = row.split('-> ').map(directionPair => {
      return directionPair.split(',').map(val => parseInt(val, 10)) as [number, number]
    })
