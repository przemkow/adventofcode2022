...
// Part2. Dynamic programming over graph - 😭
// Very slow but working solution
const totalTime = 30 - 4;
...
function dynamicProgramming(
  ...
  const key = `${nodeName}__${timeLeft}__${openValves}__${isElephant}`;
  if (cache.has(key)) {
...
export function solutionPart2() {
  const answer = dynamicProgramming("AA", totalTime, 0, false);
